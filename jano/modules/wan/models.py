...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            WanAttentionBlock(cross_attn_type, dim, ffn_dim, num_heads,
                              window_size, qk_norm, cross_attn_norm, eps, idx)
    ...
    def forward(
        ...
        if mask_manager is not None:
            seq_len = mask_manager.get_seq_len()
        
        # x = torch.cat([
        #     torch.cat([u, u.new_zeros(1, seq_len - u.size(1), u.size(2))],
        #               dim=1) for u in x
        # ]) 
        x = x[0] # 这是一个padding的操作，在只有一个视频样本的情况下，完全可以忽略
        ...
        with get_masked_timer("dit_fw"):
            if mask_manager is not None:
                # print(f"{get_timestep()} | {x.shape=} {mask_manager.get_seq_len()=}", flush=True)
