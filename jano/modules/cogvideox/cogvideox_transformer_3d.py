...
from jano.modules.cogvideox.attention_processor import CogVideoXAttnProcessor2_0, CogVideoXAttnProcessor_jano
...
class CogVideoXTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def forward(
        ...
        # print(f"{encoder_hidden_states.shape=} {hidden_states.shape=}", flush=True)
        # exit()
        ...
        if mask_manager is None:
            ...
            if mask_manager.get_seq_len() != 0:
                hidden_states = mask_manager.apply_sequence_mask(hidden_states)
            else:
                ValueError("all frozen!")
        
        # print(f"After apply mask: {hidden_states.shape=}", flush=True)
        ...
        with get_masked_timer("dit"):
            ...
            # print(f"output: {hidden_states.shape=}", flush=True)
            if mask_manager is not None:
                hidden_states = mask_manager.process_masked_output(hidden_states, "output", -1)
