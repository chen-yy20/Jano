...
class MaskManager:
    ...
    def generate_mask(self, combined_score):
        ...
        print(f"{self.latent_mask.shape=} {self.sequence_mask.shape=}", flush=True)
    ...
    def transform_mask(
        ...
        """
        将latent mask转换为token mask (CogVideoX 1.5版本)
        
        Args:
            latent_mask: 形状为 [num_frames, height, width] 的mask
            patch_size: spatial patch size = 2
            patch_size_t: temporal patch size = 2  
            aggregation_method: patch内聚合方法 ("max", "min", "mode")
        
        Returns:
            token_mask: 形状为 [sequence_length] 的1维mask
        """
        latent_mask = latent_mask[0, :]
        num_frames, height, width = latent_mask.shape
        
        p = patch_size  # 2
        p_t = patch_size_t  # 2
        
        # 按照patch_embed的3D patching方式进行变换
        # 重塑为patch groups
        mask_patches = latent_mask.reshape(
            num_frames // p_t,     # 22 // 2 = 11
            p_t,                   # 2
            height // p,           # 96 // 2 = 48
            p,                     # 2
            width // p,            # 170 // 2 = 85
        ...
        # 重新排列维度，模仿patch_embed中的permute操作
        # 对应patch_embed中的 permute(0, 1, 3, 5, 2, 4, 6)但去掉batch和channel维度
        mask_patches = mask_patches.permute(0, 2, 4, 1, 3, 5)  # [11, 48, 85, 2, 2, 2]
        
        # Flatten patch内的所有值
        patch_shape = mask_patches.shape  # [11, 48, 85, 2, 2, 2]
        mask_patches = mask_patches.contiguous().view(
            patch_shape[0], patch_shape[1], patch_shape[2], -1
        )  # [11, 48, 85, 8] (2*2*2=8个值per patch)
        
        # 在每个patch内进行聚合
        if aggregation_method == "max":
            aggregated = mask_patches.max(dim=-1)[0]  # [11, 48, 85]
        elif aggregation_method == "min":
            aggregated = mask_patches.min(dim=-1)[0]
        elif aggregation_method == "mode":
            # 计算众数
            aggregated = torch.mode(mask_patches, dim=-1)[0]
        else:
            raise ValueError(f"Unknown aggregation method: {aggregation_method}")
        
        # Flatten所有patch维度，得到sequence mask
        sequence_mask = aggregated.flatten()  # [11*48*85] = [44880]
        sequence_mask = sequence_mask.to(torch.int8)
        
        # 计算各个等级的序列长度
        self.full_seq_len = sequence_mask.shape[0]  # 序列长度 44880
    ...
    def process_kv_sequence(self, tensor: torch.Tensor, txt_seq_len: int, name: str, layer_idx: int) -> torch.Tensor:
        if self.step_level == 0:
            return tensor
        
        # 2, 2, 48, 45106, 64
        state_key = f"{name}_{layer_idx}"
        txt = tensor[:, :, :txt_seq_len, :]
        x = tensor[:, :, txt_seq_len:, :]
        B, N, S, D = x.shape
        ...
        if self.step_level == 3:            
            ...
            if get_timestep() == GlobalEnv.get_envs("warmup_steps") + 1:
                print(f"Stored {state_key}, {static_data.shape=}, {medium_data.shape=}"
            ...
            result = torch.cat([x, static_kv], dim=2)
        elif self.step_level == 1:
            # 恢复（从CPU转回GPU如果需要）
            ...
            result = torch.cat([x, medium_kv, static_kv], dim=2)
        
        result = result.reshape(B, N, -1, D)
        # print(f"{result.shape=}", flush=True)
        tensor = torch.cat([txt, tensor], dim=2)
        return tensor
