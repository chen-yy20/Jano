...
from jano.stuff import get_timestep
...
def jano_teacache_forward(
        ...
        if self.enable_teacache:
            ...
            if self.cnt == 0 or self.cnt == self.num_steps-1:
                ...
                self.accumulated_rel_l1_distance += rescale_func(((modulated_inp-self.previous_modulated_input).abs().mean() / self.previous_modulated_input.abs().mean()).cpu().item())
                if self.accumulated_rel_l1_distance < self.rel_l1_thresh:
                    print(f"{get_timestep()} | {self.accumulated_rel_l1_distance=} / {self.rel_l1_thresh}", flush=True)
                    # jano 判定
                    if use_jano:
            ...
            if self.cnt == self.num_steps:
                self.cnt = 0    
        # print(f"{get_timestep()} | {should_calc=}")  
        with get_timer("tea_DiT"):
            ...
            if self.enable_teacache:
                if not should_calc and self.previous_residual is not None:
                    print(f"{get_timestep()} | Teacache: SKIP.", flush=True)
                    hidden_states += self.previous_residual
                else:
                    # ori_hidden_states = hidden_states.clone()
                    ori_hidden_states = get_active_hidden_states(hidden_states, use_jano).clone()  
                    ...
                    if should_store:
                        self.previous_residual = get_active_hidden_states(hidden_states, use_jano) - ori_hidden_states
                        print(f"{get_timestep()} | Teacache: stored {self.previous_residual.shape=}", flush=True)
            else:
                for index_block, block in enumerate(self.transformer_blocks):
                    if torch.is_grad_enabled() and self.gradient_checkpointing:
